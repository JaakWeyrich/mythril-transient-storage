...
// This contract illustrates that writing a value to a symbolic index and trying to retrieve that value with a concrete index will fail in transient storage, as a dictionary is used to model it
...
contract HashCheckContract {
    ...
    function checkAndSend(uint256 userNumber) public {
        assembly {
            tstore(userNumber, 123)
            
            // Load the value at the calculated storage index
            let valueAtStorage := tload(0x1)
